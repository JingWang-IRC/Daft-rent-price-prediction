...
findsublink_type2 = re.compile(r'<a href="(.*?)">')
...
def getData(baseurl, savepath):
    ...
    for i in range(0, 44):
        ...
        suburl = []
        for item in soup.find_all('li', class_="SearchPage__Result-gg133s-2 itNYNv"):

            suburl_type1 = item.find_all('li', class_="SubUnits__Item-sc-150tj2u-2 iKhaLF")
            print(len(suburl_type1))

            if len(suburl_type1):
                print("fetch sub link (type1)")
                for subitem in suburl_type1:
                    subitem = str(subitem)
                    link = re.findall(findlink, subitem)
                    prefix = "https://www.daft.ie/"
                    link = prefix + link[0]
                    suburl.append(link)
            else:
                print("fetch sub link (type2)")
                subitem = str(item)
                link = re.findall(findsublink_type2, subitem)
                prefix = "https://www.daft.ie/"
                link = prefix + link[0]
                suburl.append(link)
...
def getData_suburl(suburl):
    ...
    for i in suburl:
        ...
        # for loop is not necessary, I just reused previous code
        for item in soup.find_all('div', class_="PropertyPage__MainFlexWrapper-sc-14jmnho-0 cqxrhg"):
            ...
            if len(code_text) == 0:
                print("no district code information")
                break
            ...
            if len(type_str) == 0:
                print("no house type information")
                break
        ...
        for item in soup.find_all('div', class_="PropertyPage__MainFlexWrapper-sc-14jmnho-0 cqxrhg"):
            ...
            if numBathroom >= 0:
                ...
                print("no bathroom information")
            data.append(numBathroom)

            furnished_list = re.findall(findFurnished, item)
            furnished = 0
            if len(furnished_list) == 0:
                furnished = 1
            elif furnished_list[0] == "Yes" or furnished_list[0] == "yes" or \
                    furnished_list[0] == "Optional" or furnished_list[0] == "optional":
                furnished = 1
            elif furnished_list[0] == "No" or furnished_list[0] == "no":
                furnished = 0
            # print("whether furnished: ", furnished)
            if furnished >= 0:
                counter += 1
            else:
                print("no furnish information")
            ...
            if transport >= 0:
                ...
                print("no transport information")
        ...
        for item in soup.find_all('ul', class_="PropertyDetailsList__PropertyDetailsListContainer-sc-1cjwtjz-0 bnzQrB"):
            ...
            if extraSpace >= 0:
                ...
                print("no extra space information")
